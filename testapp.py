...
# [核心修正] 強化評量類型的定義與具體出題策略
# 學術依據：Airasian & Russell (2007) - Classroom Assessment
ASSESSMENT_TYPES = {
    'placement': {
        'label': '安置性評量 (Placement)',
        'desc': '評估學生是否具備學習新單元所需的「先備知識」。',
        'prompt_instruction': """
        **核心目標**：檢測「先備知識 (Prerequisite Knowledge)」。
        **出題策略**：
        1. 請勿直接測驗本單元的內容，而是測驗「學習本單元之前必須具備的能力」。
    ...
    'diagnostic': {
        ...
        'desc': '偵測學生常見的迷思概念與學習困難點。',
        'prompt_instruction': """
        **核心目標**：偵測「迷思概念 (Misconceptions)」。
        **出題策略**：
        1. 題目的重點在於「誘答項 (Distractors)」的設計。
        2. 錯誤選項不能是隨機產生的，必須對應學生常見的特定錯誤邏輯（例如：以為小數點位數越多數值越大）。
        3. 請在題目中包含易混淆的概念比較。
        """
    },
    'formative': {
        'label': '形成性評量 (Formative)',
        'desc': '監控學習進度，提供鷹架以促進概念理解。',
        'prompt_instruction': """
        **核心目標**：提供「教學鷹架 (Scaffolding)」。
        **出題策略**：
        1. 題目範圍鎖定於本單元的當前教學內容。
        2. 難度應落在「近側發展區 (ZPD)」，即中等偏難，需要思考但非不可解。
        3. 詳解必須具備引導性，教導學生如何從題目線索推導出答案。
        """
    },
    'summative': {
        'label': '總結性評量 (Summative)',
        'desc': '評斷單元學習目標的達成程度與遷移能力。',
        'prompt_instruction': """
        **核心目標**：驗證「精熟程度 (Mastery)」。
        **出題策略**：
        1. 題目應涵蓋本單元的所有重要概念（廣度）。
        2. 包含應用題與跨概念的綜合題（深度）。
        3. 難度分佈：請混合基礎題與進階挑戰題，以鑑別不同程度的學生。
        """
    }
}
